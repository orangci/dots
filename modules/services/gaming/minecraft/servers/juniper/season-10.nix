...
let
  inherit (lib)
    ...
    my
    ;
  cfg = config.modules.services.gaming.minecraft.juniper-s10;
  packwiz = pkgs.fetchPackwizModpack {
    url = "https://git.orangc.net/c/minecraft-modpacks/raw/commit/1eae17c2d8722ef4c63b37a876e3e9cba196adc4/juniper-s10/pack.toml";
    packHash = "sha256-KF5nP/P78BmQO6XmuYRWVan7/p9V8pZDkTekQBgVdsA=";
    # dummy: pkgs.lib.fakeHash
  };
in
{
  options.modules.services.gaming.minecraft.juniper-s10 = {
  ...
  config = mkIf cfg.enable {
    ...
    modules.services.infrastructure.caddy.virtualHosts = my.mkCaddyEntry "mc-map" (
      cfg.port - 2000
    ) false;
    ...
    modules.security.sops.secrets.juniper-discord-bot-token = {
      owner = "minecraft";
      path = "/var/secrets/juniper-discord-bot-token";
    };
    modules.security.sops.secrets.juniper-in-game-chat-webhook = {
      owner = "minecraft";
    ...
    services.minecraft-servers.servers.juniper = {
      ...
      extraStartPre = ''
        ...
        bot_token=$(cat ${config.modules.security.sops.secrets.juniper-discord-bot-token.path})
      ...
      serverProperties = {
        ...
        bug-report-link = "https://${flakeSettings.domains.primary}/gh/minecraft-modpacks/issues/new";
