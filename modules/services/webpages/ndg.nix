...
let
  inherit (lib) mkIf mkMerge my;
  cfg = config.modules.services.webpages.ndg;
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    modules.services.infrastructure.cloudflared.ingress =
      my.mkCloudflaredIngress cfg.subdomain cfg.port;
    modules.services.infrastructure.caddy.virtualHosts = mkMerge [
      (my.mkCaddyEntry cfg.subdomain cfg.port true)
      {
