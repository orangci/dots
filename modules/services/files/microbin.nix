...
let
  inherit (lib)
    mkIf
    ;
  cfg = config.modules.services.files.microbin;
in
{
  options.modules.services.files.microbin = lib.my.mkServerModule {
    name = "Microbin";
    subdomain = "bin";
  };

  config = mkIf cfg.enable {
    modules.security.sops.secrets.microbin-env.path = "/var/secrets/microbin-env";
    services.microbin = {
      enable = true;
