...
let
  inherit (lib)
    ...
    singleton
    ;
  cfg = config.modules.services.files.copyparty;
  commonPerms = {
    access = {
      r = "*";
...
in
{
  ...
  options.modules.services.files.copyparty = lib.my.mkServerModule {
    name = "Copyparty";
    subdomain = "files";
  };
  ...
  config = mkIf cfg.enable {
    ...
    modules.security.sops.secrets.copyparty-password = {
    ...
    services.copyparty = {
      ...
      accounts.${users.sysadmin.username}.passwordFile =
        config.modules.security.sops.secrets.copyparty-password.path;
      volumes = {
        "/" = {
          path = "/srv/files";
