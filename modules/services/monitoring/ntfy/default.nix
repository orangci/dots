...
in
{
  options.modules.services.monitoring.ntfy = lib.my.mkServerModule { name = "Ntfy"; } // {
    acl = mkOption {
  ...
  config = mkIf cfg.enable {
    ...
    modules.security.sops.secrets.ntfy-env.path = "/var/secrets/ntfy-env";
    ...
    services.ntfy-sh = {
      enable = true;
      environmentFile = config.modules.security.sops.secrets.ntfy-env.path;
      settings = {
