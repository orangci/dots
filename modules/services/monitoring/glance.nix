...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = [
      "d /run/glance - - - -"
      "d /run/glance/assets - - - -"
      "L+ /run/glance/assets/font.css - - - - ${pkgs.writeText "font.css" ''
            ...
            columns = [
              {
                ...
                widgets = [
                  {
                    type = "search";
                    search-engine = "https://${config.modules.services.tools.searxng.subdomain}.${flakeSettings.domains.tailnet}/search?q={QUERY}";
                  }
                  (mkIf config.modules.services.infrastructure.technitium.enable {
                    type = "dns-stats";
                    ...
                    service = "technitium";
                    token = {
                      _secret = config.modules.security.sops.secrets.technitium-api-token.path;
                    };
                  })
                  {
                    type = "clock";
              ...
              }
              {
                ...
                widgets = [
                  ...
                  (mkIf config.modules.services.monitoring.scrutiny.enable {
                    type = "custom-api";
                    title = "Drive Health";
                    title-url = "https://${config.modules.services.monitoring.scrutiny.subdomain}.${flakeSettings.domains.tailnet}";
                    url = "https://${config.modules.services.monitoring.scrutiny.subdomain}.${flakeSettings.domains.tailnet}/api/summary";
                    cache = "1h";
                    method = "GET";
                    options = {
                    ...
                    template = ''
                      ...
                      {{- range $sorted }}
                        ...
                        <div style="margin-top: 1rem; display: flex; justify-content: space-between; align-items: center;">
                          <a href="http://${config.modules.services.monitoring.scrutiny.subdomain}.${flakeSettings.domains.tailnet}/web/device/{{ $wwn }}" target="_blank">
                            <div>
