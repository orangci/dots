...
let
  inherit (lib)
    ...
    singleton
    ;
  cfg = config.modules.services.productivity.forgejo;
...
in
{
  options.modules.services.productivity.forgejo.renovate.enable = mkEnableOption "Renovate bot";
  config = mkIf cfg.renovate.enable {
    ...
    services.renovate = {
      ...
      credentials = {
        RENOVATE_TOKEN = config.modules.security.sops.secrets."renovate/forgejo-token".path;
        RENOVATE_GITHUB_COM_TOKEN = config.modules.security.sops.secrets."renovate/github-token".path;
      };
    };
  };
}
