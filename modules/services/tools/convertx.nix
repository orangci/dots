...
let
  inherit (lib)
    ...
    singleton
    ;
  cfg = config.modules.services.tools.convertx;
in
{
  options.modules.services.tools.convertx = lib.my.mkServerModule {
    name = "ConvertX";
    subdomain = "convert";
  ...
  config = mkIf cfg.enable {
    modules.security.sops.secrets.convertx-env.path = "/var/secrets/convertx-env";
